...
function calcPoints(guessA: number, guessB: number, actualA: number, actualB: number): number {
  ...
  if (aMatch && bMatch) return 5
  if (aMatch || bMatch) return 2
  return 0
}
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="bg-white/5 border border-white/10 p-1 mb-4 flex-wrap gap-0.5">
              ...
              {scheduleDays.map((day) => (
                <TabsTrigger key={day.date} value={day.date} className="data-[state=active]:bg-zinc-800 text-[9px] sm:text-[10px] px-1.5 sm:px-2 py-1">
                  {day.label}
                </TabsTrigger>
              ))}
            </TabsList>
                  ...
                  {day.matches.map((fixture) => {
                    ...
                    return (
                      <div key={fixture.matchNumber} className={cn(
                        ...
                        <div className="flex items-center justify-between px-3 py-1.5 bg-white/[0.02]">
                          <div className="flex items-center gap-2">
                            ...
                            {!locked && !matchDTO?.isFact && (
                              <span className="text-[9px] text-yellow-400/60 font-mono tabular-nums">
                                {countdown}
                              </span>
                            )}
                          </div>
                        ...
                        {/* Simulation Override Row */}
                        <div className={cn(
                          "flex items-center justify-between px-3 py-2.5",
                          isSet ? "bg-green-500/5" : ""
                        )}>
                            ...
                            {matchDTO?.isFact && !unlockedFacts[matchDTO.id] ? (
                              ...
                              <>
                                <Input
                                  ...
                                  className="w-10 sm:w-12 h-9 sm:h-10 text-center text-base sm:text-lg font-bold bg-zinc-900 border-white/10"
                                  placeholder="-"
                                />
                                <span className="text-white/20 font-black text-xs sm:text-sm">VS</span>
                                <Input
                              ...
                              <div className="flex items-center gap-2 text-xs">
                                ...
                                <span className={cn(
                                  ...
                                  matchDTO.scoreA != null && matchDTO.scoreB != null
                                    ? calcPoints(guess.scoreA, guess.scoreB, matchDTO.scoreA, matchDTO.scoreB) >= 5
                                      ? "text-yellow-400"
                                      : "text-green-400"
                                    : "text-white/30"
                                )}>
                                  {matchDTO.scoreA != null && matchDTO.scoreB != null
                                    ? `+${calcPoints(guess.scoreA, guess.scoreB, matchDTO.scoreA, matchDTO.scoreB)}`
                                    : "?"}
                                </span>
                              </div>
                            ) : (
                    ...
                    <div key={groupId} className="space-y-2">
                      ...
                      <div className="space-y-1.5">
                        {(standings as any[]).slice(0, 4).map((s: any, idx: number) => {
                          ...
                          return (
                            <div key={s.teamId} className="flex items-center justify-between text-[10px]">
                              <div className="flex items-center gap-1.5">
                                <span className="text-white/20 w-2.5 text-[9px]">{idx + 1}</span>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-muted-foreground">Both exact = 5pts, one exact = 2pts.</p>
        </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-white/5 border-b border-white/10">
                ...
                <th className="text-right px-3 py-2.5 text-[9px] font-bold uppercase tracking-widest text-white/40">Pts</th>
                <th className="text-center px-3 py-2.5 text-[9px] font-bold uppercase tracking-widest text-white/40 hidden sm:table-cell">Both</th>
                <th className="text-center px-3 py-2.5 text-[9px] font-bold uppercase tracking-widest text-white/40 hidden sm:table-cell">One</th>
              </tr>
            </thead>
            <tbody>
