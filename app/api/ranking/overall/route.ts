...
const PREDICTION_WEIGHT = 0.25
const POOL_WEIGHT = 0.30
const GAMES_WEIGHT = 0.15
const CHIPS_WEIGHT = 0.10
...
export async function GET() {
  ...
  const chipBalances = await prisma.chipBalance.findMany({
    select: { userId: true, lifetimeEarnings: true, balance: true },
  })
  ...
  const cbMap = new Map(chipBalances.map((c) => [c.userId, { lifetimeEarnings: c.lifetimeEarnings, balance: c.balance }]))
  ...
  for (const user of users) {
    ...
    if (cb && cb.lifetimeEarnings > maxLifetimeEarnings) maxLifetimeEarnings = cb.lifetimeEarnings
  }
  ...
  const leaderboard = users
    .map((user) => {
      ...
      const rawChips = cb ? cb.lifetimeEarnings : 0
      ...
      const chipsSkill = maxLifetimeEarnings > 0 ? (rawChips / maxLifetimeEarnings) * 100 : 0

      const adjustment = adjMap.get(user.id) ?? 0

      const overallScore = connectionSkill * CONNECTION_WEIGHT + predictionSkill * PREDICTION_WEIGHT + poolSkill * POOL_WEIGHT + gameSkill * GAMES_WEIGHT + chipsSkill * CHIPS_WEIGHT + adjustment
      ...
      return {
        ...
        chips: { lifetimeEarnings: rawChips, skill: Math.round(chipsSkill * 100) / 100 },
      }
    })
